            ...
            <tbody class="divide-y divide-white/5">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 text-center">
                        ...
                        <a href="<?= base_url('admin/packages/delete/'.$row['id']) ?>"
...
<div id="pacModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 hidden p-4">
    <div class="bg-slate-900 p-6 rounded-2xl w-full max-w-sm border border-white/10">
        <form id="pacForm" method="post" action="<?= base_url('admin/packages/add') ?>">
            ...
            <input type="text" name="badge" id="badge" placeholder="Badge (e.g. Popular)" class="w-full bg-slate-800 p-3 rounded-xl mb-3 text-white" required>
...
<!-- package_channels টেবিলের জন্য এই মোডালটা আগে ছিলই না — এখান থেকেই প্ল্যানে চ্যানেল অ্যাড/এডিট করা যাবে -->
...
function editPackage(data) {
    document.getElementById('pacForm').action = "<?= base_url('admin/packages/edit/') ?>" + data.id;
...
function openChannelsModal(planId, assignedIds) {
    document.getElementById('channelsForm').action = "<?= base_url('admin/packages/assign_channels/') ?>" + planId;
